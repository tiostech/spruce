...
Rcpp::List spruceCpp(uint treetype, Rcpp::NumericMatrix& input_x, Rcpp::NumericMatrix& input_y, Rcpp::NumericMatrix& input_z, 
  ...
  try {
    ...
    // However, the initialization syntax { } used here initializes the std::unique_ptr with a null pointer.
    ...
    if (verbose) {
      ...
      // Rcpp::Rcout is an output stream provided by Rcpp for printing to the R console.
    } else {
      verbose_out = new std::stringstream; 
      // allowing output to be stored in memory as a string.
    }
    ...
    // Initialize data 
    if (use_sparse_data) {
      // data = std::make_unique<DataSparse>(sparse_x, input_y, input_z, variable_names, num_rows, num_cols);
    } else {
      data = std::make_unique<DataRcpp>(input_x, input_y, input_z, input_test_x, variable_names, num_rows, num_cols);
    }
    ...
    if (strcmp(e.what(), "User interrupt.") != 0) {
      Rcpp::Rcerr << "Error: " << e.what() << " spruce will EXIT now." << std::endl;
    }
    return result;
  }
