...
Forest::Forest() :
    ...
    NAN), importance_mode(DEFAULT_IMPORTANCE_MODE), optim_mode(DEFAULT_OPTIM_MODE), regularization_usedepth(false), progress(0) {
}
...
void Forest::initCpp(std::string dependent_variable_name, MemoryMode memory_mode, std::string input_file, uint mtry,
    ...
    std::string load_forest_filename, ImportanceMode importance_mode, OptimMode optim_mode, uint min_node_size, uint min_bucket,
  ...
  init(loadDataFromFile(input_file), mtry, output_prefix, num_trees, seed, num_threads, importance_mode, optim_mode,
...
void Forest::initR(std::unique_ptr<Data> input_data, uint mtry, uint num_trees, std::ostream* verbose_out, uint seed,
    uint num_threads, ImportanceMode importance_mode, OptimMode optim_mode,  uint min_node_size, uint min_bucket,
  ...
  // Call other init function
  init(std::move(input_data), mtry, "", num_trees, seed, num_threads, importance_mode, optim_mode, min_node_size, min_bucket,
...
void Forest::init(std::unique_ptr<Data> input_data, uint mtry, std::string output_prefix,
    uint num_trees, uint seed, uint num_threads, ImportanceMode importance_mode, OptimMode optim_mode, uint min_node_size, uint min_bucket,
    bool prediction_mode, bool sample_with_replacement, const std::vector<std::string>& unordered_variable_names,
...
void Forest::grow() {
  ...
  for (size_t i = 0; i < num_trees; ++i) {
    ...
    trees[i]->init(data.get(), mtry, num_samples, tree_seed, &deterministic_varIDs, tree_split_select_weights,
        importance_mode, optim_mode, min_node_size, min_bucket, sample_with_replacement, memory_saving_splitting, splitrule, &case_weights,
