...
spruce <- function(formula = NULL, data = NULL, test_data = NULL, num.trees = 500, mtry = NULL,
                   ...
                   reference.variable.name = NULL, 
                   status.variable.name = NULL, 
                   classification = NULL, x = NULL, y = NULL, z = NULL, ...) {
    ...
    if (is.null(formula)) {
      if (is.null(dependent.variable.name)) {
        ...
        if (is.null(status.variable.name)) {
          ...
          x <- data[, !(colnames(data) %in% c(dependent.variable.name, reference.variable.name)), drop = FALSE]
          z <- data[, reference.variable.name, drop = TRUE]
          test_x = test_data[, !(colnames(test_data) %in% c(dependent.variable.name, reference.variable.name)), drop = TRUE] %>% as.data.frame()
        } else {
  ...
  }else if(splitrule == "crystal_v2oppabs_0p5"){
    if(treetype == 3){
      splitrule.num <- 25
      print("splitrule = 25")
    }else{
      stop("Error: crystal_v2oppabs_0p5 splitrule applicable to regression data only.")
    }
  }else if(splitrule == "crystal_v2oppabs_1p0"){
    if(treetype == 3){
      splitrule.num <- 26
      print("splitrule = 26")
    }else{
      stop("Error: crystal_v2oppabs_1p0 splitrule applicable to regression data only.")
    }
  }else if(splitrule == "crystal_v2oppabs_2p0"){
    if(treetype == 3){
      splitrule.num <- 27
      print("splitrule = 27")
    }else{
      stop("Error: crystal_v2oppabs_2p0 splitrule applicable to regression data only.")
    }
  }else if(splitrule == "crystal_v2oppabs_3p0"){
    if(treetype == 3){
      splitrule.num <- 28
      print("splitrule = 28")
    }else{
      stop("Error: crystal_v2oppabs_3p0 splitrule applicable to regression data only.")
    }
  }else {
